...
<head>
    ...
    <link rel="stylesheet" href="{{asset('assets/admin/css/custom.css')}}?v=2.5">
...
<body class="footer-offset" id="{{ config('app.mode') == 'demo' ? 'demo' : '' }}">
...
<script>
    (function () {
        ...
        function pollBranchStoreNotifications() {
            $.ajax({
                url: '{{route('branch.get-store-data')}}',
                type: 'GET',
                cache: false,
                dataType: 'json',
                headers: {
        ...
        pollBranchStoreNotifications();
        setInterval(pollBranchStoreNotifications, 10000);
    })();
</script>
<script>
...
<script>
    ...
    function print_invoice(order_id) {
        $.get({
            url: '{{url('/')}}/branch/orders/pos-invoice/'+order_id,
            dataType: 'json',
            beforeSend: function () {
                $('#loading').show();
            },
