<div style="width:410px;" class="mx-auto">
    <div class="text-center pt-2 mb-3">
        <h2>{{ Helpers::get_business_settings('store_name') ?? Helpers::get_business_settings('restaurant_name') }}</h2>
        ...
        @if($order->customer)
            <div class="col-12 text-dark pb-2">
                ...
                @if($order->order_type != 'pos')
                    <div>
                        {{translate('Address')}}
                        : {{ is_array($order->delivery_address) ? ($order->delivery_address['address'] ?? '') : (isset($order->delivery_address) ? (json_decode($order->delivery_address, true)['address'] ?? '') : '') }}
                    </div>
                @endif
            </div>
        @endif
    ...
    <dl class="row text-dark mt-2">
        ...
        <dd class="col-6 text-right">{{ Helpers::set_symbol($sub_total + $total_tax) }}</dd>
        ...
        <dd class="col-6 text-right">
            - {{ Helpers::set_symbol($order['loyalty_discount_amount'] ?? 0) }}
        </dd>
        @endif
        ...
        @if($order->order_type == 'pos_delivery' && ($order->delivery_charge ?? 0) > 0)
        <dt class="col-6">{{translate('delivery')}} {{translate('fee')}}:</dt>
        <dd class="col-6 text-right">{{ Helpers::set_symbol($order->delivery_charge) }}</dd>
        @endif
