        ...
        <div class="card">
            <div class="card-header bg-light pages-card-header">
                <h6 class="mb-0 fw-semibold d-flex align-items-center gap-2">
                    <i class="tio-document me-2"></i>{{ translate('about_us') }}
                </h6>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.business-settings.about-us') }}" method="post" id="about-us-form">
                    ...
                    <div class="pages-editor">
                        ...
                        <div class="form-group mb-0 lang_form {{ $lang != ($defaultLang ?? 'ar') ? 'd-none' : '' }}"
                            ...
                            @endif
                            <label class="input-label d-block mb-2">{{ translate('about_us') }} ({{ strtoupper($lang) }})</label>
                            <div id="{{ $lang }}_editor" class="min-h-15">{!! \App\CentralLogics\Helpers::sanitizeHtmlForDisplay($contentByLang[$lang] ?? '') !!}</div>
                            <input type="hidden" name="about_us[{{ $lang }}]" id="{{ $lang }}_hiddenArea" value="">
                        </div>
                        @endforeach
                        ...
                        <div class="d-flex flex-wrap justify-content-end gap-2 mt-4 pt-3 border-top">
                            ...
                            <button type="submit" class="btn btn-primary px-4 min-w-120">{{ translate('save') }}</button>
                        </div>
                    </div>
                </form>
            </div>
