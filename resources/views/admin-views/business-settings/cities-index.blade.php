    ...
    <div class="content container-fluid">
        <div class="mb-3 d-flex align-items-center justify-content-between flex-wrap gap-2">
            <h2 class="text-capitalize mb-0 d-flex align-items-center gap-2">
                <i class="tio-map"></i> {{ translate('cities') }}
            </h2>
            <button type="button" class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#citiesInstructionsModal">
                <i class="tio-book-outlined"></i> {{ translate('help_cities_btn') }}
            </button>
        </div>

        @include('admin-views.partials._help-instructions-modal', ['modalId' => 'citiesInstructionsModal', 'titleKey' => 'help_cities_title', 'pageKey' => 'help_cities_page'])

        <div class="card">
            <div class="card-header border-0">
                <h5 class="card-title">{{ translate('cities') }}</h5>
            </div>
            <div class="card-body">
                ...
                @endif
                <form action="{{ route('admin.business-settings.cities.store') }}" method="post" class="mb-4">
                    @csrf
                    @if($language)
                            ...
                            @foreach(json_decode($language) as $lang)
                                <div class="col-12 col-md-6 col-lg-5 lang_form {{ $lang != $default_lang ? 'd-none' : '' }}" id="{{ $lang }}-form">
                                    <label class="form-label">
                                        {{ translate('city') }} ({{ strtoupper($lang) }}) @if($lang == $default_lang)<span class="text-danger">*</span>@else
                                    ...
                                    <input type="text" name="name[{{ $lang }}]" id="{{ $lang }}_name" class="form-control" placeholder="{{ translate('add_new_city') }}" maxlength="100" value="{{ old('name.'.$lang) }}"
                                           {{ $lang == $default_lang ? 'required' : '' }}>
                                </div>
                            @endforeach
                        @else
                            <div class="col-12 col-md-6 col-lg-5">
                                <label class="form-label">{{ translate('city') }} ({{ strtoupper($default_lang) }}) <span class="text-danger">*</span></label>
                                <input type="text" name="name[{{ $default_lang }}]" class="form-control" placeholder="{{ translate('add_new_city') }}" required maxlength="100" value="{{ old('name.'.$default_lang) }}">
                            </div>
                        @endif
                        <div class="col-12 col-md-6 col-lg-4">
                            <label class="form-label">{{ translate('delivery_areas') }}</label>
                            <select name="area_id" class="form-control">
                                <option value="">{{ translate('select') }}</option>
                                @foreach($areas as $area)
                                    <option value="{{ $area->id }}" {{ old('area_id') == $area->id ? 'selected' : '' }}>{{ $area->getNameByLang($default_lang) ?? $area->name_en ?? $area->name_ar ?? '-' }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-border table-thead-bordered table-nowrap table-align-middle card-table">
                        <thead class="thead-light">
                            <tr>
                                <th>#</th>
                                <th>{{ translate('delivery_areas') }}</th>
                                <th>{{ translate('city') }} ({{ strtoupper($default_lang) }})</th>
                        ...
                        <tbody>
                            @forelse($cities as $key => $city)
                                <tr>
                                    ...
                                    <td>
                                        <span class="city-area-display">@if($areas->isNotEmpty() && $city->relationLoaded('area') && $city->area){{ $city->area->getNameByLang($default_lang) ?? '-' }}@else - @endif</span>
                                    </td>
                                    <td>
                                        <span>{{ $city->getNameByLang($default_lang) ?? '-' }}</span>
                                    </td>
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-soft-primary" data-toggle="modal" data-target="#editCityModal-{{ $city->id }}">
                                            <i class="tio-edit"></i>
                                        </button>
                                        <form action="{{ route('admin.business-settings.cities.destroy', $city->id) }}" method="post" class="d-inline" onsubmit="return confirm('{{ translate('Are you sure') }}?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-4">{{ translate('no_cities_added') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="modal fade" id="editCityModal-{{ $city->id }}" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('admin.business-settings.cities.update', $city->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <select name="area_id" class="form-control">
                                    <option value="">{{ translate('select') }}</option>
                                    @foreach($areas as $a)
                                        <option value="{{ $a->id }}" {{ $city->area_id == $a->id ? 'selected' : '' }}>
                                            {{ $a->getNameByLang($default_lang) ?? '-' }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            @if($language)
                                @foreach(json_decode($language) as $editLang)
                                    <div class="mb-3">
                                        <label class="form-label">
                                            {{ translate('city') }} ({{ strtoupper($editLang) }})
                                            @if($editLang == $default_lang)<span class="text-danger">*</span>@else
                                            <button type="button" class="btn btn-sm btn-outline-primary ms-2 translate-btn"
                                                    data-source-lang="{{ $default_lang }}" data-target-lang="{{ $editLang }}"
                                                    data-source-id="city_edit_{{ $city->id }}_{{ $default_lang }}_name"
                                                    data-target-id="city_edit_{{ $city->id }}_{{ $editLang }}_name" data-is-html="0"
                                                    title="{{ translate('Auto translate from') }} {{ strtoupper($default_lang) }}">
                                                <i class="tio-globe"></i> {{ translate('Auto translate') }}
                                            </button>
                                            @endif
                                        </label>
                                        <input type="text" name="name[{{ $editLang }}]" id="city_edit_{{ $city->id }}_{{ $editLang }}_name" class="form-control"
                                               value="{{ $city->getNameByLang($editLang) ?? '' }}" maxlength="100"
                                               {{ $editLang == $default_lang ? 'required' : '' }}>
                                    </div>
                                @endforeach
                            @else
                                <div class="mb-3">
                                    <label class="form-label">{{ translate('city') }} <span class="text-danger">*</span></label>
                                    <input type="text" name="name[{{ $default_lang }}]" class="form-control"
                                           value="{{ $city->getNameByLang($default_lang) ?? '' }}" maxlength="100" required>
                                </div>
                            @endif
                        </div>
