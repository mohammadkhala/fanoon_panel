...
@section('title', translate('best_selling_products'))

@section('content')
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="text-capitalize mb-0 d-flex align-items-center gap-2">
                <img width="20" src="{{asset('assets/admin/img/icons/order_report.png')}}" alt="{{ translate('best_selling_products') }}">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.report.best-selling-products') }}" method="GET">
                    <div class="bg-light rounded p-3">
                        <div class="row align-items-end g-3">
                            ...
                            <div class="col-sm-12 col-xl-4 d-flex gap-2 align-items-end">
                                <a href="{{ route('admin.report.best-selling-products') }}" class="btn btn--reset min-w-120 flex-grow-1">{{ translate('clear') }}</a>
                                <button type="submit" class="btn btn-primary min-w-120 flex-grow-1">{{ translate('show') }}</button>
                            </div>
                        </div>
                        <div class="row mt-2">
                            <div class="col-md-6 pt-2">
                                <strong>{{ translate('total') }} {{ translate('products') }}: <span>{{ count($data) }}</span></strong>
                            </div>
                            <div class="col-6 pt-2">
                                <div class="hs-unfold float-right">
                                    <a class="js-hs-unfold-invoker btn btn-sm btn-white"
                                       href="{{ route('admin.report.export-best-selling-products', ['start_date' => $startDate, 'end_date' => $endDate, 'limit' => $limit]) }}">
                        ...
                        <div class="table-responsive datatable-custom">
                            <table class="table table-hover table-border table-thead-bordered table-nowrap table-align-middle card-table">
                                ...
                                <tr>
                                    ...
                                    <th>{{ translate('qty') }} {{ translate('sold') }}</th>
                                ...
                                @foreach($data as $key => $row)
                                    <tr>
                                        ...
                                        <td>
                                            @if($row['product'])
                                                <a href="{{ route('admin.product.view', ['id' => $row['product']->id]) }}">{{ $row['product']->name ?? '—' }}</a>
                                            @else
                                                —
                                            @endif
                                        </td>
                                        <td>{{ $row['total_qty'] }}</td>
                                        <td>{{ \App\CentralLogics\Helpers::set_symbol($row['total_amount']) }}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
