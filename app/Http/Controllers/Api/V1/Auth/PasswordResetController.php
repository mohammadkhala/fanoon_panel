        ...
        if (isset($customer)) {
            ...
            if ($request['type'] == 'phone') {
                return response()->json([
                    'errors' => [['code' => 'sms', 'message' => translate('SMS is not configured. Phone password reset is disabled.')]],
                ], 403);
            }

            if ($request['type'] == 'email') {
                if (config('app.mode') == 'local') {
                    Log::info('Password reset OTP sent (local mode – not sent by email)', ['email' => $customer['email']]);
                } elseif (config('app.mode') == 'live') {
                    try {
