...
class LoginSetupController extends Controller
{
    ...
    public function loginSetup(): Factory|\Illuminate\Foundation\Application|View|Application
    {
        ...
        $loginOptions = json_decode($loginOptionsValue) ?? new \stdClass();

        $socialMediaLoginValue = $this->loginSetup->where(['key' => 'social_media_for_login'])->first()?->value ?? '{}';
        $socialMediaLoginOptions = json_decode($socialMediaLoginValue) ?? new \stdClass();
    ...
    public function loginSetupUpdate(Request $request): RedirectResponse
    {
        ...
        if ($request->has('social_media_login')){
            if(!$request->has('google')){
                Toastr::error(translate('Please active Google login when Social Media Login is active.'));
                return back();
            }
        }
    ...
    public function checkActiveSocialMedia(Request $request): JsonResponse
    {
        ...
        $clientId = $appleLoginService['client_id'] ?? null;
        $teamId = $appleLoginService['team_id'] ?? null;
        $keyId = $appleLoginService['key_id'] ?? null;
        $serviceFile = $appleLoginService['service_file'] ?? null;
    ...
    private function InsertOrUpdateLoginData($key, $value): void
    {
        ...
        if ($loginSetup) {
            ...
            $this->loginSetup->create(['key' => $key['key'], 'value' => $value['value']]);
        }
    }
}
